...
int main(){
   ...
   if(c != NULL && c2 != NULL){
      ...
      c = destroi_conjunto(c);
      c2 = destroi_conjunto(c2);
      c3 = destroi_conjunto(c3);
      c4 = destroi_conjunto(c4);
      sub = destroi_conjunto(sub);
      inter = destroi_conjunto(inter);
      dif = destroi_conjunto(dif);
      uni = destroi_conjunto(uni);

      if(c != NULL || c2 != NULL || c3 != NULL || c4 != NULL || sub != NULL || inter != NULL
       || dif != NULL || uni != NULL){
         printf("Erro! conjuntos nao foram destruidos corretamente!\n");
         return 1;
      }

      printf("Testes passaram - conjunto\n");
      return 0;
   }
   return 1;
}