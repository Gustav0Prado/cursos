...
// #define UNROLL 4
// #define getDiag(A, i, j, n, k) ( (j-i > (k/2) || j-i < -(k/2)) ? 0.0 : A[k*i+((k/2)+(j-i))] )

// //Multiplica matriz simetrica com vetor
// extern inline void multMatSimVet(SistLinear_t *restrict SL, double *restrict vet, double *restrict res, int n){
...
// extern inline double multVetVet(double *restrict v1, double *restrict v2, int n){
//    double soma = 0.0;
//    double soma2 = 0.0;
...
// extern inline void mulVetVetDiagonal (double *restrict vetDiagonal, double *restrict v2, double *restrict res, int n) {
//    for (int i = 0; i < n-n%UNROLL; i+=UNROLL){
...
// // res = v1 +m*v2
// extern inline void somaVetVet(double *restrict v1, double m, double *restrict v2, double *restrict res, int n){
//    for (int i = 0; i < n-n%UNROLL; i+=UNROLL){
//       res[i]   = v1[i]   + m*v2[i];
...
// //retorna v1*A*v2
// extern inline double multVetMatVet(double *restrict v1, SistLinear_t *restrict SL, double *restrict v2, int n){
...
// //res = v1 + mAv2
// extern inline void somaVetMatxVet(SistLinear_t *restrict SL, double *restrict v1, double *restrict v2, double m, double *restrict res, int n){
//    double *aux = malloc(n*sizeof(double));
//    memset(aux, 0, n*sizeof(double));
...
// extern inline double normamax (double *restrict v1, double *restrict v2, int n){
//    double diff;
//    double maior = 0.0;
...
// // retorna max ( |xi - xi-1| )
// extern inline double normamaxAbs (double *restrict v1, double *restrict v2, int n){
//    double diff;
//    double maior = 0.0;
...
// extern inline double residuoDiag(SistLinear_t *restrict SL, double *restrict b, double *restrict x, double *restrict r){
...
//    for(int i = 0; i < n-n%UNROLL; i+=UNROLL){
//       for(int j = 0; j < n; ++j){
//          r[i]   += getDiag(SL->Diag.A, i, j, n, k)   * x[j];
//          r[i+1] += getDiag(SL->Diag.A, (i+1), j, n, k) * x[j];
//          r[i+2] += getDiag(SL->Diag.A, (i+2), j, n, k) * x[j];
//          r[i+3] += getDiag(SL->Diag.A, (i+3), j, n, k) * x[j];
//       }
//       r[i]   -= SL->Diag.b[i];
//       r[i+1] -= SL->Diag.b[i+1];
//       r[i+2] -= SL->Diag.b[i+2];
